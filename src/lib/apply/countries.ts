/**
 * ISO 3166-1 alpha-2 countries, English short names, sorted A to Z.
 *
 * `eea` marks the European Economic Area (EU 27 plus Iceland, Liechtenstein and
 * Norway). Portugal only requires a tax representative from people who live
 * OUTSIDE the EEA, so the flag is read off the country of residence, never off
 * the passport. Switzerland and the United Kingdom are deliberately not EEA.
 *
 * Names with a hyphen (Guinea-Bissau, Timor-Leste) are proper nouns, not the
 * punctuation dash the copy rules forbid. Leave them alone.
 *
 * Generated with Intl.DisplayNames; edit by hand only to add or remove rows.
 */
...
export const COUNTRIES: readonly Country[] = [
  ...
  { code: "YE", name: "Yemen" },
  { code: "ZM", name: "Zambia" },
  { code: "ZW", name: "Zimbabwe" },] as const;
...
export function isEea(code: string | undefined): boolean {
  return countryByCode(code)?.eea === true;
}
