...
/**
 * "In progress": one card per order that is paid and not complete, plus the
 * ones completed in the last seven days, which keep their place with a
 * Delivered badge and a confetti burst on every page load. Which orders
 * qualify is decided by `isInProgress` in order-status.ts; the page filters
 * and this file lays out.
 *
 ...
 * by a swipe. Server component; the controls and the confetti are the two
 * client islands.
 *
 * Every card ends on See more, a link to `?order=<id>` on the same page,
 * which the server answers by rendering the order modal.
 */
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[0.9rem] text-navy-soft">
            {completed && order.completed_at ? copy.completedOn(formatDate(order.completed_at)) : stageLabel}
          </p>
        </div>
        {delivered && (
      ...
      <div className="relative mt-auto pt-6">
        <Link
          ...
          scroll={false}
          aria-label={copy.seeMoreAria(name)}
          className="inline-flex h-11 items-center gap-2 rounded-full bg-navy px-6 text-sm font-medium text-white transition-colors duration-200 hover:bg-gold hover:text-navy focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2 focus-visible:ring-offset-white"
        >
          {copy.seeMore}
          <ArrowRight className="size-4" aria-hidden />
        </Link>
